...
int64_t max_square_product(int64_t nums[20][20], int64_t r, int64_t c) {
  ...
  // up/down
  if (r - 3 >= 0) {
    products[0] *= nums[r-1][c];
    products[0] *= nums[r-2][c];
    products[0] *= nums[r-3][c];
  }

  // left/right
  if (c - 3 >= 0) {
    products[1] *= nums[r][c-1];
    products[1] *= nums[r][c-2];
  ...
  return max_prod * nums[r][c];
}

int64_t max_diagonal_product(int64_t nums[20][20], int64_t r, int64_t c) {
  int64_t products[2];
  products[0] = 1;
  products[1] = 1;
  ...
  if (r - 3 >= 0 && c - 3 >= 0) {
    products[0] *= nums[r-1][c-1];
    products[0] *= nums[r-2][c-2];
    products[0] *= nums[r-3][c-3];
  }

  // diagonally up right / down left
  if (r - 3 >= 0 && c + 3 <= 19) {
    products[1] *= nums[r-1][c+1];
    products[1] *= nums[r-2][c+2];
...
int main(int argc, char** argv) {
  ...
  // input copied from https://projecteuler.net/problem=11
  ...
  // Get numerical values for input and store in matrix
  int64_t nums[20][20];
  ...
  for (int r = 0; r < 20; r++) {
    for (int c = 0; c < 20; c++) {
      ...
      int64_t diagonal_product = max_diagonal_product(nums, r, c);

      if (square_product > max_num) {
        max_num = square_product;
      }

      if (diagonal_product > max_num) {
        max_num = diagonal_product;
      }
    }
  }
